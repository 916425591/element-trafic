  ...
  <div>
    ...
    <modal v-model="eqModal" :title="title" :mask-closable="false" width="600" class-name="vertical-center-modal">
      <i-form ref="formCustom" :model="formCustom" :rules="ruleCustom" :label-width="80">
        <Row>
          <i-col span="12">
            <form-item label="装备类型" prop="resTypeName">
              <i-input type="text" v-model="formCustom.resTypeName" @on-focus="selectEquipmentType"></i-input>
            </form-item>
          </i-col>
          <i-col span="12">
          ...
          <i-col span="24">
            <form-item label="装备职能" prop="functions">
              <i-input v-model="formCustom.functions" type="textarea" placeholder="请输入装备职能"></i-input>
            </form-item>
          </i-col>
        </Row>
      </i-form>
...
export default {
          ...
          render: (h, params) => {
            return h('div', [
              h('Button', {
                ...
                on: {
                  click: () => {
                    this.editEquipment(params.index)
                  }
                }
              }, '编辑'),
              h('Poptip', {
  ...
  methods: {
    ...
    equipmentQuery () {
      ...
      axios({
          method: 'post',
          url: this.url + '/plan/content/queryPlanContent4ResEquipmentPlanByPage',
          data: info
      }).then(
          response => {
    ...
    editEquipment (index) {
      ...
      //装备详情查询
      let equipmentId = this.tableData[index].id;
      const info = {
        userCode: this.userCode,
        id: equipmentId
      }
      axios({
          method: 'get',
          url: this.url + '/plan/content/getPlanContent4ResEquipmentDetail',
          params: info
      }).then(
          response => {
            if ( response.data.code === 200 ) {
    ...
    handleSubmit (name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          ...
          //判断是否新增或者修改
          if (info.id === null) {
            url = this.url + '/plan/content/addPlanContent4ResEquipment';
          } else {
            url = this.url + '/plan/content/modifyPlanContent4ResEquipment';
          }
          //提交装备估算信息
          axios({
              method: 'post',
