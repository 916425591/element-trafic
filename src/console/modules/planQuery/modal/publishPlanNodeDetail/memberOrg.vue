                ...
                <li v-for="(item,index) in memberList" :key="index">
                    <Card style="" dis-hover>
                        ...
                        <div class="ds-member-body">
                            ...
                            <div>
                                <i-input v-model="item.functions" placeholder="请输入职责。" type="textarea" :rows="7" readonly></i-input>
                            </div>
                        </div>
                    </Card>
                </li>
                <li v-if="this.memberList.length===0">
                    <Card style="" dis-hover>
                        <p slot="title">
                            <Icon type="home"></Icon>
                            成员单位
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getAllMemberOrg() {
                const url = this.url + '/plan/PlanContent4MemberOrg/queryFBPlanContent4MemberOrgByPage?pageSize='+ this.pageSize + '&&currentPage=' + this.page
                const info = {
                    nodeId: this.nodeId,
                    planId: this.planId,
                    userCode: this.userCode
                            ...
                            res.list.forEach((v, i) => {
                                this.memberList.push({
                                    ...
                                    person: {
                                        ...
                                        name: v.name,
                                        mobile: v.mobile,
                                        personName: v.personName ? v.personName : '暂无负责人'
                                    }
                                });
                                this.dutyList.push({
                                    functions: v.orgDuty ? v.orgDuty : ''
