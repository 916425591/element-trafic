    ...
    <div class="ds-widget-box ds-box" :style='height' :data-json="tableHeight">
        <div class="ds-widget-title">
            ...
            <Row class="ds-fload-right">
                <i-col span="24" class="ds-button-box">
                    ...
                    <Button type="ghost" v-else @click="close_team">关闭</Button>
        ...
        <div class="ds-table-box">
            <Form ref="formCustom" :model="formItem" :rules="ruleCustom" :label-width="110">
                ...
                <FormItem label="预案类型名称:" prop="planName">
                    <i-input v-model="formItem.planName" style="width:300px;"  :disabled="ds_edit"></i-input>
                </FormItem>
                <FormItem label="预案编码:" prop="planCode">
                    <i-input v-model="formItem.planCode" style="width:300px;" :disabled="ds_edit"></i-input>
                </FormItem>
                <FormItem label="有效性:">
...
export default {
    ...
    props: ['expertType', 'planName', 'planCode', 'id', 'parentName', 'parentId', 'switchStatus', 'children', 'queryCode'],
    data () {
    ...
    computed: {
        ...
        ds_edit() {
            return this.$store.state.planContent.planStatus
        },
        pageSize() {
    ...
    methods: {
            ...
            this.$refs[name].validate((valid) => {
                if (valid) {
                    let resTypeInfo = {
                        ...
                        name: this.formItem.planName,
                        id: this.formItem.id,
                        parentId: this.formItem.parentId,
                        code: this.formItem.planCode
                    }
                    axios({
                        method: 'post',
                        url: this.url('/platform/planType/savePlanType'),
                        data: resTypeInfo
                    }).then(
                        response => {
        ...
        delete_equipment () {
            ...
            axios({
                method: 'get',
                url: this.url('/platform/planType/removePlanType'),
