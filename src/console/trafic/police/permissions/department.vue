    ...
    <div>
        <el-form ref="theOfficer" :model="theOfficer" label-width="80px" class="permissions">
            ...
            <el-col :span="12" style="margin-left: 80px">
                ...
                <el-button type="primary" @click="newdepar('new')">添加部门</el-button>
            </el-col>
        </el-form>
        <div>
            <el-table :data="tableData" show-header style="width: 100%">
                <el-table-column prop="id" label="部门编码"></el-table-column>
                <el-table-column prop="name" label="上级部门编码"></el-table-column>
                <el-table-column prop="fullName" label="部门全称"></el-table-column>
                <el-table-column prop="name" label="部门简称"></el-table-column>
                <el-table-column  label="部门级别">
                    <template slot-scope="scope">
                        <span v-if="scope.row.level == 0">支队</span>
                        <span v-if="scope.row.level == 1">区级</span>
                        <span v-if="scope.row.level == 2">市级</span>
                        <span v-if="scope.row.level == 3">厅级</span>
                    </template>
...
<script>
    ...
    import  mainenaceNew from  '@/common/components/scMaintenance/maintenance'
    export default {
        name: 'permissions',
        ...
        methods:{
            ...
            handleDelete(index, row) {
                ...
                const url =this.getUrl+ '/dept/delete/'+row.id
                const dataList={
                    paraMap:'',
                    pageHelper:''
                }
            ...
            newdepar(data,row){
                this.datalagio = {};
              if(data=='new'){//这是新增
                  this.mainUrl='/dept/insert'//新增
              }else if(data=='editor'){//编辑
                  this.mainUrl='/dept/update' //编辑
                 this.datalagio =row;
              }
              this.visible = true;
            }
